...
def get_coverage_data(request):
    ...
    with cube() as dc:
        ...
        if subsetting_crs not in cfg.published_CRSs:
            raise WCS2Exception(
                "Invalid subsettingCrs: %s" % subsetting_crs,
        ...
        if output_crs not in cfg.published_CRSs:
            raise WCS2Exception(
                "Invalid outputCrs: %s" % output_crs,
        ...
        for subset in subsets:
            ...
            if dimension == "time":
                ...
                try:
                    ...
                    raise WCS2Exception(
                        "Invalid subsetting axis %s" % subset.dimension,
        ...
        if len(scales) != len(set(subset.axis.lower() for subset in scales)):
            ...
            raise WCS2Exception(
                "Duplicate scales for ax%ss: %s"
                % ("i" if len(duplicate_axes) == 1 else "e", ", ".join(duplicate_axes)),
        ...
        for scale in scales:
            ...
            if axis in ("time", "k"):
                raise WCS2Exception(
                    "Cannot scale axis %s" % scale.axis,
                    WCS2Exception.INVALID_SCALE_FACTOR,
                    locator=scale.axis,
                )
            else:
        ...
        if not request.format:
            ...
            try:
                ...
                raise WCS2Exception(
                    "Unsupported format: %s" % request.format,
        ...
        geobox = geometry.GeoBox(
            scaler.size.x, scaler.size.y, affine, cfg.crs(output_crs)
        )
