...
class WCS1GetCoverageRequest:
    ...
    def __init__(self, args):
        ...
        if not self.product or not self.product.wcs:
            raise WCS1Exception(
                "Invalid coverage: %s" % self.product_name,
        ...
        if args["format"] not in cfg.wcs_formats_by_name:
            raise WCS1Exception(
                "Unsupported format: %s" % args["format"],
        ...
        if self.request_crsid not in cfg.published_CRSs:
            raise WCS1Exception(
                "%s is not a supported CRS" % self.request_crsid,
                WCS1Exception.INVALID_PARAMETER_VALUE,
                locator="CRS parameter",
                valid_keys=list(cfg.published_CRSs),
            )
        ...
        if "response_crs" in args:
            ...
            if self.response_crsid not in cfg.published_CRSs:
                raise WCS1Exception(
                    "%s is not a supported CRS" % self.response_crsid,
                    WCS1Exception.INVALID_PARAMETER_VALUE,
        ...
        if "time" not in args:
            ...
            if len(self.times) == 0:
                raise WCS1Exception(
                    ...
                    locator="TIME parameter",
                    valid_keys=[
                        d.strftime("%Y-%m-%d") for d in self.product.ranges["time_set"]
                    ],
                )
        ...
        self.geobox = geometry.GeoBox(
            self.width, self.height, self.affine, self.request_crs
        )
...
def get_coverage_data(req):
    ...
    with cube() as dc:
        ...
        stacker = DataStacker(req.product, req.geobox, req.times, bands=req.bands)
        ...
        if (
            req.product.max_datasets_wcs > 0
            and n_datasets > req.product.max_datasets_wcs
        ):
            raise WCS1Exception(
