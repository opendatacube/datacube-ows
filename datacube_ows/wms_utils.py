...
def _get_geobox(args, src_crs, dst_crs=None):
    ...
    if dst_crs is not None:
        minx, miny, maxx, maxy = _bounding_pts(
            minx, miny, maxx, maxy, width, height, src_crs, dst_crs=dst_crs
        )
...
def zoom_factor(args, crs):
    ...
    minx, miny, maxx, maxy = _bounding_pts(
        minx, miny, maxx, maxy, width, height, crs, dst_crs=geo_crs
    )
...
def get_arg(
    args, argname, verbose_name, lower=False, errcode=None, permitted_values=None
):
    ...
    if permitted_values:
        if fmt not in permitted_values:
            raise WMSException(
                "%s %s is not supported" % (verbose_name, fmt),
                errcode,
                locator="%s parameter" % argname,
                valid_keys=permitted_values,
            )
...
def parse_wms_time_string(t, start=True):
    if t.upper() == "PRESENT":
        ...
        default = (
            datetime(1970, 1, 1)
            if start
            else datetime(1970, 12, 31, 23, 23, 59, 999999)
...
class GetMapParameters(GetParameters):
    def method_specific_init(self, args):
        ...
        if not self.style:
            raise WMSException(
                "Style %s is not defined" % style_r,
                WMSException.STYLE_NOT_DEFINED,
...
def cosine_of_solar_zenith(lat, lon, utc_dt):
    ...
    result = math.sin(latitude_rad) * math.sin(solar_decl_rad) + math.cos(
        latitude_rad
