...
def test_legend_parser_urllegend(prelegend_style):
    url = "http://whatevs"
    prelegend_style.parse_legend_cfg({"show_legend": True, "url": url})
...
def test_parse_colorramp_legend_beginend():
    ramp = ColorRamp(
        None, {"range": [-1.0, 2.5], "legend": {"begin": "0.0", "end": "2.0"}}
    )
    assert ramp.legend_begin == Decimal("0.0")
...
def test_parse_colorramp_legend_ticksevery():
    ramp = ColorRamp(
        ...
        {
            "range": [-1.0, 2.5],
            "legend": {"begin": "0.0", "end": "2.0", "ticks_every": "0.4"},
        },
    )
    assert ramp.ticks == [
        Decimal("0.0"),
...
def test_parse_colorramp_legend_tickcount():
    ramp = ColorRamp(
        ...
        {
            "range": [-1.0, 2.5],
            "legend": {"begin": "0.0", "end": "2.0", "tick_count": 2},
        },
    )
...
def test_parse_colorramp_legend_find_end():
    ramp = ColorRamp(
        ...
        {
            "color_ramp": [
                {"value": 999, "color": "#000000", "alpha": 0.0},
                {"value": 1000, "color": "#000000"},
                {"value": 2500, "color": "#BA7500"},
                {"value": 6500, "color": "#BF4000"},
                {"value": 51500, "color": "#EF1000"},
            ],
            "legend": {
                "show_legend": True,
                "begin": "1000",
    ...
    ramp = ColorRamp(
        None,
        {
            ...
            "legend": {
                ...
                "tick_labels": {
                    "default": {"prefix": "p", "suffix": "s"},
                    "0.3": {},
                    "0.8": {"prefix": "", "suffix": "Zz"},
                    "0.9": {"suffix": "Zz"},
                    "1.1": {
                        "prefix": "#",
                    },
                    "1.7": {"prefix": "pre", "label": "fixe"},
                    "1.9": {
                        "label": "o",
                    },
                    "2.0": {
