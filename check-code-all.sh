...
DB_DUMP="$ROOT_DIR/integration_tests/cfg/test_db.sql"

export PGPASSWORD=$POSTGRES_PASSWORD

set -ex

if [ -n "$GENERATE_DB_DUMP" ]; then
...
EOF
  ...
  datacube-ows-update -E owspostgis --schema

  # Update range tables for configured layers.
  datacube-ows-update

  pg_dumpall --exclude-database="template*" -c --if-exists -h $POSTGRES_HOSTNAME -U $POSTGRES_USER -p $POSTGRES_PORT > $DB_DUMP
fi

# Restore database from dump for performance reasons.
psql -h $POSTGRES_HOSTNAME -U $POSTGRES_USER -p $POSTGRES_PORT < $DB_DUMP
